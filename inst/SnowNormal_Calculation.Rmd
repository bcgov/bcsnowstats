...
3. If a station had less that 10 years of data, no normals were calculated. 

Details regarding the application of these thresholds is shown below.

#### 1. Stations with 20 or More Years of Data

Stations with at least 20 years of data (where a year is only counted if there was at least 80% of the daily data present within the snow accumulation and melt period) was used without modification to calculate normals. The only data pre-processing step in this case was the calculation of daily mean SWE values in cases where raw hourly values were available.
...
aswe_n_1981t2010_r <- read.csv(paste0("G:/Snow/Normals_2020/snownormals_1991t2020_rprocess/", "aswe_firstfiftheenth_1981t2010", ".csv")) %>%
#aswe_n_1991t2020_r <- normals_1t15_unfold %>%
  dplyr::select(station_id, normal_swe_mean, m_d) %>%
  dplyr::mutate(date = as.Date(m_d, "%m-%d")) %>%
  dplyr::mutate(date = format(date, "%b%d")) %>%
  dplyr::rename(STATIONID = station_id, normal_1981t2010_r = normal_swe_mean) %>%
