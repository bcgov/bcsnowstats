...
snow_stats <- function(data, normal_min, normal_max, data_id, force = FALSE) {
  ...
  df_hist <- data %>%
    ...
    dplyr::group_by(station_id, m_d) %>% # Group by the station ID and the month/day
  ...
  df_time_2 <- df_time %>%
    ...
    dplyr::select(station_id, wr, percent_available) %>%
    unique() %>%
  ...
  numberyears <- df_time_2 %>%
    ...
    dplyr::select(station_id, numberofyears) %>%
    unique()

  # Bind stats regarding the number of years available together
  df_time <- dplyr::full_join(daterange, numberyears, by = c("station_id"))
  ...
  df_stat_1 <- dplyr::full_join(df_stat_date, dates, by = c("station_id", "m_d"))
  ...
  df_hist_SWE <- df_hist %>%
    dplyr::group_by(station_id, m_d) %>%
    dplyr::select(m_d, values_stats, station_id) %>%
    dplyr::rename(mean_SWE = values_stats) %>%
    tidyr::nest() %>%
    dplyr::select(m_d, data, station_id)

  # append the normal to the stats
  df_stats_final <- dplyr::full_join(df_stats_all, df_hist_SWE, by = c("station_id", "m_d")) %>%
